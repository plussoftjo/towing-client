...
export default class StepSix extends React.Component {
    ...
    _start_route() {
        ...
        // this.props.manger.mapLayout.route_directions = true;
        // this.props.manger.mapLayout.route_marker = true;
        map.animateToRegion({
            latitude:locations.map_coords.latitude,
            longitude:locations.map_coords.longitude,
            latitudeDelta:0.015,
        ...
        this.props.manger._change_step(8);
    }

    componentDidMount() {
        let self = this;
        let start_route = function() {
            ...
            /** Manger to other step */
            self.props.manger._change_step(6.1);
        ...
        self.props.socketmanger.when_start_the_service(start_route);
    }
        ...
        return [
            <View key="TopText" style={{position:'absolute',top:0,left:0,width:'100%'}}>
                <View style={{padding:20,paddingTop:'20%'}}>
                    <Animatable.Text iterationCount="infinite" duration={5000} animation="flash" style={{fontWeight:'700',fontSize:16,textAlign:'center',color:'rgba(0,0,0,0.6)'}}>Meet driver, The driver is on the way</Animatable.Text>
            ...
            <View style={{borderTopRightRadius:50,borderTopLeftRadius:50,backgroundColor:'white',width:'100%',shadowColor: "#000",shadowOffset: {width: 0,height: 2,},shadowOpacity: 0.25,shadowRadius: 3.84,elevation: 5,borderColor:'#0D0D0D',borderWidth:2}}>
                ...
                <View style={{flexDirection:'row',justifyContent:'center',alignContent:'center',alignItems:'center',marginTop:10,borderTopColor:'rgba(0,0,0,0.2)',borderTopWidth:0.3,paddingTop:5}}>
                    <View style={{flexDirection:'row',width:'50%',justifyContent:'center',alignItems:'center',alignContent:'center'}}>
                        <Icon type="MaterialCommunityIcons" name="map-marker-distance" style={{fontSize:22,color:'rgba(0,0,0,0.8)'}} />
                        <Text style={{fontWeight:'700',color:'rgba(0,0,0,0.8)'}}> {this.props.processor.distance.distance.text}</Text>
                    </View>
                    <View style={{flexDirection:'row',width:'50%',justifyContent:'center',alignItems:'center',alignContent:'center'}}>
                        <Icon type="AntDesign" name="clockcircleo" style={{fontSize:22,color:'rgba(0,0,0,0.8)'}} />
