...
@inject('settings')
@observer
export default class StepEleven extends React.Component {
    constructor(props) {
        super(props);
        this.end_service = this.end_service.bind(this);
    }

    async end_service() {
        axios.post(this.props.settings.serverUri + 'api/order/change_state_to_four',{order_id:this.props.processor.service.order_id}).then(response => {}).catch(err => {console.log(err);});
        await AsyncStorage.removeItem('order');
        this.props.manger._reMapLayoutValue();
        this.props.processor.reset();
